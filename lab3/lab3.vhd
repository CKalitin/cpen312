...
architecture rtl of lab3 is
	...
	signal alarm_active  : std_logic := '0'; -- When time matches alarm, LED0 stays on for 1001 (9+1) seconds
	signal alarm_counter : std_logic_vector(5 downto 0) := "000000"; -- 6-bit counter counts down from 9 to 0 (60 seconds total)
...
begin
	...
	-- Checks every second if current time equals alarm time.
	-- When matched, alarm_active goes high for 10 seconds.
	process(clk)
	begin
		if rising_edge(clk) then
			if one_sec_tick = '1' then
				...
				if alarm_active = '1' then
					if alarm_counter = "000000" then
						alarm_active <= '0'; -- 10 seconds elapsed, turn off alarm
					else
						alarm_counter <= alarm_counter - 1;
					end if;
				end if;

				-- Match detection: if current time = alarm time, (re)start alarm
				-- Checked AFTER decrement so a new match overrides a running countdown
				if ss_ones = alarm_ss_ones and ss_tens = alarm_ss_tens and
				   mm_ones = alarm_mm_ones and mm_tens = alarm_mm_tens and
				   hh_ones = alarm_hh_ones and hh_tens = alarm_hh_tens and
				   is_pm = alarm_is_pm then
					alarm_active  <= '1';
					alarm_counter <= "001001"; -- 9 in binary, counts down to 0 = 10 seconds
				end if;
			end if;
		end if;
	end process;
	...
	-- LED0 = alarm ringing indicator (on for 60 seconds after match)
	LED(0) <= alarm_active;
